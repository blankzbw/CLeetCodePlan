...
static：静态；应用场景：修饰3种数据1.函数内部的变量；
...
const：常量的定义，只读的变量；（中看不中用，说不能改变，其实还是能够用地址改变，因为本质还是存在了可读可写的变量区，只是在编码时不能显示的进行修改）

volatile：告知编译器编译方法的关键字，对当前变量不进行优化编译；修饰变量的值的修改，不仅仅可以通过软件，也可以通过其他方式（硬件外部的用户）；
